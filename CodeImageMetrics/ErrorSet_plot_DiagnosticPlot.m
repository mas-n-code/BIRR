function ErrorSet_plot_DiagnosticPlot(ErrorSet,sProp,sf1,DTag)
%Plots locations of tumors and of where they are expected to be on the
%image
  nested_Diagnosticplot(ErrorSet.E1,sProp,sf1,DTag,['E1 ']);
  try
  nested_Diagnosticplot(ErrorSet.E2,sProp,sf1,DTag,['E2 ']);
  nested_Diagnosticplot(ErrorSet.E4,sProp,sf1,DTag,['E4 ']);
  nested_Diagnosticplot(ErrorSet.E12,sProp,sf1,DTag,['E12 ']);
  catch
      disp('Single Experiment Mode')
  end

    function ExpSet=nested_Diagnosticplot(ExpSet,sProp,sf1,DTag,s_name)
    Single_DiagnosticPlot(ExpSet.P_P1,sProp,sf1,DTag,s_name);
    %Single_DiagnosticPlot(ExpSet.P_P2,sProp,sf1,s_name);    <-Only printing one, no aparent need to plot one for each phantom
    %Single_DiagnosticPlot(ExpSet.P_P3,sProp,sf1,s_name);
    end
