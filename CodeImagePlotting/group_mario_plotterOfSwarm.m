function group_mario_plotterOfSwarm(ExpSet,sProp)
%PLOTFIGURE
...
plot(rand(length(s3.R_tValues.tumor),1)/2 +i,s3.R_tValues.tumor,'ko','MarkerFaceColor',[0.8 0.2 0.2].^(0.25)); i=i+1; plotVL(i)
...
plot(rand(length(s3.R_tValues.fibro),1)/2 +i,s3.R_tValues.fibro,'ko','MarkerFaceColor',[0.1 0.8 0.2].^(0.25)); i=i+1; plotVL(i)
...
plot(rand(length(s3.R_tValues.clutter),1)/2 +i,s3.R_tValues.clutter,'ko','MarkerFaceColor',[0.15 0.15 0.8].^(0.25)); i=i+1; plotVL(i)
...
set(gca,'XTick', [0.5 1.5 2.5 3.5 4.5 5.5 6.5 7.5 8.5],...
    'XTickLabel',{'T1' 'T2' 'T3' 'F1' 'F2' 'F3' 'C1' 'C2' 'C3'});
