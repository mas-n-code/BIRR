function [SCAN]=monofun_mario_cmS(ref,data,RecSettings,graph_title)
...
% RECSETTINGs: Structure file w/
    ...
    % in meters
    % RecSettings.f_points; % Frequency points for each file Note that
    % previous datasets were recored in 601, but since 2016 we changed to 1001
...
%% Revision changes
%----------------------------------------------- 
%Version 2.1.8.0 Date:11/05/2017
%- SCAN now contains Params too
%- changed counterWise name to counterCWise

%Version 2.1.7.1 Date:04/05/2017
%- f_points added to RecSettings 
...
f_points=RecSettings.f_points; % Frequency points from the file, could be automated
...
patch_r = 1.75; % radius of the patch
...
%---Spatial Resolution---%
delta_s=(speed*100/(4*(fl_high-fl_low))); %(Mu/(2*(6e9)))=0.0121 where Mu=1.45e8
...
% savethisone([graph_title 'Raw'])  % >Save>

%% ---xx S11 calibrated and shifted ---
...
% Here i put the piece of code to time t that signal from copper
...
% savethisone(supershort_savetitle);  % >Save>
...
%
[ BSCAN2 ] = data_former( s11_win,zeros(f_points,l_size),145/57.3 ); 
...
Params.freqvals=linspace(fl_low,fl_high,f_points);
...
disp(['Employing Multi_rec_full v= ',num2str(recver)])
switch recver
    case 201
