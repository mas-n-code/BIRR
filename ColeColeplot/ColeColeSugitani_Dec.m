...
load('G:\Dropbox\ResearchMagic\RandomProyects\171101 Debye and Cole Cole\Permittivity Measurements\Measurements_July2016_TysonReimer.mat')
...
%Std and errors in meas
MeasStd.Saline.Real=std(Mes_July2016_TR.Saline.Real,0,2);
MeasStd.Saline.Imag=std(Mes_July2016_TR.Saline.Imag,0,2);

MeasStd.Glycerine.Real=std(Mes_July2016_TR.Glycerine.Real,0,2);
MeasStd.Glycerine.Imag=std(Mes_July2016_TR.Glycerine.Imag,0,2);

MeasStd.Triton30.Real=std(Mes_July2016_TR.Triton30.Real,0,2);
MeasStd.Triton30.Imag=std(Mes_July2016_TR.Triton30.Imag,0,2);
...
Ecmx_Suig(ii,:)= E_inf(ii) + (E_m(ii)-E_inf(ii))./(1+power(1j*w_freq_ang*t_tauQ(ii),1-B_betta(ii))) + (E_s(ii)-E_m(ii))./(1+1j*w_freq_ang*t_tau(ii)) - (sigma_condu(ii)./(w_freq_ang*Eo_vacuum))*1j; %(E_m-E_inf)./(1+(1j*w_freq*t_tauQ).^(1-B_betta))
 ...
 Ecmx_Suig_ConductEff(ii,:)=0-imag(Ecmx_Suig(ii,:)).*w_freq_ang*Eo_vacuum;
...
ylim([0,125]); xlim([1E9,6E9])
title('Permittivity of breast tissues and phantom surrogates from 1 GHz to 6 GHz')
ylabel('Permittivity');
...
%errorbar(Meas.Freq(1:20:end),Meas.Perm.Saline(1:20:end),MeasStd.Saline.Real(1:20:end),'^','MarkerFaceColor',RRed,'MarkerSize',5);
...
LLeg.FontSize=10;
LLeg.Position=[0.580,0.63,0.325517669025846,0.270182973089935];


%%
E_SDat6_tumor=5.10;
E_SDat6_stroma=9.16;
E_SDat6_adipose=4.45;

% plot(6E9,65,'^','Color',[.8,.2,.2]) % Maximum tumor at 6GHz [65]
% plot(6E9,35,'^','Color',[.8,.2,.2]) % Minimum tumor at 6GHz [35]

%Plot uncertainties for tumor
plot(w_freq,real(Ecmx_Suig(1,:))+(E_SDat6_tumor*3),'--','LineWidth',1,'Color',RRed)% Median Stroma +1 sd [5.10] %56 is the cell with the 6GHz frequency 
plot(w_freq,real(Ecmx_Suig(1,:))-(E_SDat6_tumor*3),'--','Color',RRed,'LineWidth',1)% Median Stroma +1 sd [5.10] %56 is the cell with the 6GHz frequency 

%Plot uncertainties for stroma
plot(w_freq,real(Ecmx_Suig(2,:))+(E_SDat6_stroma*3),'--','Color',GGren,'LineWidth',1)% Median Stroma +1 sd [5.10] %56 is the cell with the 6GHz frequency 
plot(w_freq,real(Ecmx_Suig(2,:))-(E_SDat6_stroma*3),'--','Color',GGren,'LineWidth',1)% Median Stroma +1 sd [5.10] %56 is the cell with the 6GHz frequency 

%Plot uncertainties for adipose
plot(w_freq,real(Ecmx_Suig(3,:))+(E_SDat6_adipose*3),'--','Color',BBlue,'LineWidth',1)% Median Stroma +1 sd [5.10] %56 is the cell with the 6GHz frequency 
plot(w_freq,real(Ecmx_Suig(3,:))-(E_SDat6_adipose*3),'--','Color',BBlue,'LineWidth',1)% Median Stroma +1 sd [5.10] %56 is the cell with the 6GHz frequency 
...
set(gca, ...
  ...
  'TickDir'     , 'in'     , ...
  'TickLength'  , [.02 .02]);
title('Conductivity of breast tissues and phantom surrogates from 1 GHz to 6 GHz')
...
LLeg.FontName='Times New Roman';
LLeg.FontSize=10;
LLeg.Position=[0.1517    0.63    0.3258    0.2702];

%%


Mu_SDat6_tumor=0.79;
Mu_SDat6_stroma=1.40;
Mu_SDat6_adipose=0.67;

% plot(6E9,65,'^','Color',[.8,.2,.2]) % Maximum tumor at 6GHz [65]
% plot(6E9,35,'^','Color',[.8,.2,.2]) % Minimum tumor at 6GHz [35]

%Plot uncertainties for tumor
plot(w_freq,real(Ecmx_Suig_ConductEff(1,:))+(Mu_SDat6_tumor*3),'--','LineWidth',1,'Color',RRed)% Median Stroma +1 sd [5.10] %56 is the cell with the 6GHz frequency 
plot(w_freq,real(Ecmx_Suig_ConductEff(1,:))-(Mu_SDat6_tumor*3),'--','Color',RRed,'LineWidth',1)% Median Stroma +1 sd [5.10] %56 is the cell with the 6GHz frequency 

%Plot uncertainties for stroma
plot(w_freq,real(Ecmx_Suig_ConductEff(2,:))+(Mu_SDat6_stroma*3),'--','Color',GGren,'LineWidth',1)% Median Stroma +1 sd [5.10] %56 is the cell with the 6GHz frequency 
plot(w_freq,real(Ecmx_Suig_ConductEff(2,:))-(Mu_SDat6_stroma*3),'--','Color',GGren,'LineWidth',1)% Median Stroma +1 sd [5.10] %56 is the cell with the 6GHz frequency 

%Plot uncertainties for adipose
plot(w_freq,real(Ecmx_Suig_ConductEff(3,:))+(Mu_SDat6_adipose*3),'--','Color',BBlue,'LineWidth',1)% Median Stroma +1 sd [5.10] %56 is the cell with the 6GHz frequency 
plot(w_freq,real(Ecmx_Suig_ConductEff(3,:))-(Mu_SDat6_adipose*3),'--','Color',BBlue,'LineWidth',1)% Median Stroma +1 sd [5.10] %56 is the cell with the 6GHz frequency 
