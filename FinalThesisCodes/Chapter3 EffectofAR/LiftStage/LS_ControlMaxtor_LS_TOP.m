...
f_variables='F:\UserElGuapo\DBOx\Dropbox\ResearchMagic\RandomProyects\160110 12 Effects of AR\MASTER Dataset LiftStage\1 Top\Control Scans';
...
LSSet_T.L0.P1.file_ref=Ref_files(1).name;
LSSet_T.L0.P1.file_data=TumorFibro_files(1).name;
LSSet_T.L0.P1.file_tumor=Tumor_files(1).name;

LSSet_T.L0.P1.set_ref=load(Ref_files(1).name);
LSSet_T.L0.P1.set_data=load(TumorFibro_files(1).name);
LSSet_T.L0.P1.set_tumor=load(Tumor_files(1).name);

% Second, CC experiment
LSSet_T.L0.P2.file_ref=Ref_files(2).name;
LSSet_T.L0.P2.file_data=TumorFibro_files(2).name;
LSSet_T.L0.P2.file_tumor=Tumor_files(2).name;

LSSet_T.L0.P2.set_ref=load(Ref_files(2).name);
LSSet_T.L0.P2.set_data=load(TumorFibro_files(2).name);
LSSet_T.L0.P2.set_tumor=load(Tumor_files(2).name);


cd(f_masterSets)

save TopRegion_Control LSSet_T
...
RecSettings.Iclims=[0 7e-9]; % LS limit is 5.5    RS limit was 5e-9!
...
RecSettings.Bdiam=.19; %diameter of the breast <------ particular f this exp in meters
RecSettings.wflip=0;
...
RecSettings.Arrasize=size(LSSet_T.L0.P1.set_ref,2)/2;
LSSet_T.RecSettings=RecSettings; 

%% Save REC settings
cd(f_variables)
save LS_T_RecSettings RecSettings
%%

%% Reconstruct Top Control C
...
supershort='P1 LS Top [C] Control';
a_Target=LSSet_T.L0.P1.set_data;
a_Reference=LSSet_T.L0.P1.set_ref;
[LSSet_T.L0.P1.C_Scan]=...
...
supershort='P2 LS Top [C] Control';
a_Target=LSSet_T.L0.P2.set_data;
a_Reference=LSSet_T.L0.P2.set_ref;
[LSSet_T.L0.P2.C_Scan]=...
    monofun_mario_cmS(a_Reference,a_Target,RecSettings,supershort);
...
supershort='P1 LS Top [B] Control';
a_Target=LSSet_T.L0.P1.set_tumor;
a_Reference=LSSet_T.L0.P1.set_ref;
[LSSet_T.L0.P1.TumorScan]=...
    monofun_mario_cmS(a_Reference,a_Target,RecSettings,supershort);

%CC Combo
RecSettings.counterWise=0;RecSettings.wflip=1;

supershort='P2 LS Top [B] Control';
a_Target=LSSet_T.L0.P2.set_tumor;
a_Reference=LSSet_T.L0.P2.set_ref;
[LSSet_T.L0.P2.TumorScan]=...
    monofun_mario_cmS(a_Reference,a_Target,RecSettings,supershort);
...
ImageControl1=abs(LSSet_T.L0.P1.C_Scan.S11_c_RecZoomFlip);
ImageControl2=abs(LSSet_T.L0.P2.C_Scan.S11_c_RecZoomFlip);
ImageTumor=abs(LSSet_T.L0.P1.TumorScan.S11_c_RecZoomFlip);
...
LSSet_T.Regions=Regions;

cd(f_variables); save LS_T_RegionsControlScan Regions
cd(f_masterSets); save TopRegion_Control LSSet_T
...
regions_show(ImageControl1,ImageControl2,LSSet_T.Regions,codeName1,codeName2,IM_Clim);


cd([f_variables,'\B_Regions'])

savethisone('RegionsOfSegmentation_Control1and2_LSTop')

%% 3 Calculate metrics based on regions

LSSet_T.L0.P1.ImageMetrics.Name= 'C-Scan LS-T Control P1';
LSSet_T.L0.P2.ImageMetrics.Name= 'C-Scan LS-T Control P2';
LSSet_T.L0.P1.PowImageMetrics.Name= 'C-Scan LS-T Control P1 Power';
LSSet_T.L0.P2.PowImageMetrics.Name= 'C-Scan LS-T Control P2 Power';
...
[LSSet_T.L0.P1.ImageMetrics]=I_RegionStandard(ImageControl1,Regions,LSSet_T.L0.P1.ImageMetrics,tresh_magnitude); 
savethisone('Control P1 magnitude structure mean')

% Calculate values of regions in control scan 2
[LSSet_T.L0.P2.ImageMetrics]=I_RegionStandard(ImageControl2,Regions,LSSet_T.L0.P2.ImageMetrics,tresh_magnitude); 
savethisone('Control P2 magnitude structure mean')

% Calculate values of regions in control scan 1 POWER
[LSSet_T.L0.P1.PowImageMetrics]=I_RegionStandard(ImageControl1.^2,Regions,LSSet_T.L0.P1.PowImageMetrics,IP_Clim); 
savethisone('Control P1 power structure mean')

% Calculate values of regions in control scan 2 POWER
[LSSet_T.L0.P2.PowImageMetrics]=I_RegionStandard(ImageControl2.^2,Regions,LSSet_T.L0.P2.PowImageMetrics,IP_Clim); 
...
figure;imagesc(refAverageImage.Image); colorbar; axis('square');savethisone('AverageControlImageLSTop')
refAverageImage.PowMetrics.Name='RefImageAverageControlLSTop';
...
[LSSet_T.L0.P1.PowImageMetrics]=IQ_Standard(ImageControl1.^2,refAverageImage.Image,LSSet_T.L0.P1.PowImageMetrics,refAverageImage.PowMetrics); 
savethisone([LSSet_T.L0.P1.PowImageMetrics.Name 'SSIM vs Average POw']);% Same, just saved in both Metric Files

[LSSet_T.L0.P2.PowImageMetrics]=IQ_Standard(ImageControl2.^2,refAverageImage.Image,LSSet_T.L0.P2.PowImageMetrics,refAverageImage.PowMetrics); 
savethisone([LSSet_T.L0.P2.PowImageMetrics.Name 'SSIM vs Average POw']);% Same, just saved in both Metric Files


cd(f_masterSets); save TopRegion_Control LSSet_T;

%% 3.d Calculate radar image quality metrics [based on magnitude, of course]
[LSSet_T.L0.P1.ImageMetrics]=I_ScR(LSSet_T.L0.P1.ImageMetrics);  
[LSSet_T.L0.P1.ImageMetrics]=I_TfR(LSSet_T.L0.P1.ImageMetrics);
[LSSet_T.L0.P1.ImageMetrics]=I_CtfCR(LSSet_T.L0.P1.ImageMetrics); 

[LSSet_T.L0.P2.ImageMetrics]=I_ScR(LSSet_T.L0.P2.ImageMetrics);  
[LSSet_T.L0.P2.ImageMetrics]=I_TfR(LSSet_T.L0.P2.ImageMetrics);
[LSSet_T.L0.P2.ImageMetrics]=I_CtfCR(LSSet_T.L0.P2.ImageMetrics); 
...
close all;
[LSSet_T.L0.P1] = spatialPos(LSSet_T.L0.P1,ImageControl1);
[LSSet_T.L0.P2] = spatialPos(LSSet_T.L0.P2,ImageControl2);
...
close all;
% Generate eSummary 
[LSSet_T.L0.P1]=eSummaryMaker(LSSet_T.L0.P1);
[LSSet_T.L0.P2]=eSummaryMaker(LSSet_T.L0.P2);
cd(f_masterSets); save TopRegion_Control LSSet_T;

% Export such eSummary
cd([f_variables,'\E_ExcelFiles']) 
estructExceler(LSSet_T.L0.P1.ImageMetrics.eSummary,[LSSet_T.L0.P1.ImageMetrics.Name,'eSummary'],1);
estructExceler(LSSet_T.L0.P2.ImageMetrics.eSummary,[LSSet_T.L0.P2.ImageMetrics.Name,'eSummary'],1);



estructExceler(LSSet_T_LEAN.L0Summary,['Top Control','LOSummary'],2);

%% LEAN
removeFields={'set_ref','set_data','set_tumor','C_Scan','TumorScan'};
LSSet_T_LEAN=LSSet_T;
LSSet_T_LEAN.L0.P1=rmfield(LSSet_T.L0.P1,removeFields);
LSSet_T_LEAN.L0.P2=rmfield(LSSet_T.L0.P2,removeFields);
cd(f_variables);
save TopRegion_Control_LEAN LSSet_T_LEAN;

%% UNDERWORK  5 Create plots that have IQM as controls. UNDERWORK
...
[LScontrolSummaryError]=ContolSet_table(LSSet_T_LEAN.L0);
LSSet_T_LEAN.L0Summary.GroupTable=LScontrolSummaryError.GroupTable;
LSSet_T_LEAN.L0Summary.table2exp=LScontrolSummaryError.table3exp;
LOsummary=LSSet_T_LEAN.L0Summary;
save LiftStageControlSummary LOsummary 

cd(f_variables); save TopRegion_Control_LEAN LSSet_T_LEAN;

% save to excel files
cd([f_variables,'\E_ExcelFiles']);  
writetable(LSSet_T_LEAN.L0Summary.GroupTable,['Top Control-','GroupTable','.xlsx'])
writetable(LSSet_T_LEAN.L0Summary.table2exp,['Top Control-','table2exp','.xlsx'])
...
%% Do the same for top layer

%
%% Load variables if required
%load('F:\UserElGuapo\Google Drive\masterSets\LiftStage\1 Top\TopRegion_Control.mat')
%cd(f_variables);load TopRegion_Control_LEAN;
