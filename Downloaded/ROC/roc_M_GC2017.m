function ROCout=roc_GC2014(varargin)
...
verbose2=false;  %[M] set verboxe 2 to false
...
tr=repmat('-',1,100);
n_ones=length(x(x(:,2)==1)); %number of unhealthy subjects
n_zeros=length(x(x(:,2)==0)); %number of healthy subjects
z=sortrows(x,1);
if threshold==0
...
YounesI=a; %<- [M] mario starts playing with variables, try to generate  Youden index
...
YounesI=a(:,2)+a(:,1)-1; %<- [M] Younes calculation                          <--
if hbar>ubar
...
size(yroc)
%}
% [M] area removal Area=trapz(xfit,yfit); %estimate the area under the curve
Area=trapz(flip(xroc),flip(yroc));

%[M] Add perfcurve calculation fix 
%[xroc_perf,yroc_perf,T_perf,Area_perf] = perfcurve(x(:,2),x(:,1),1);
[xroc_perf,yroc_perf,T_perf,Area_perf] = perfcurve(x(:,2),x(:,1),1); 
Area=Area_perf;

%standard error of area
Serror=mario_SE_foAUC(Area,n_ones,n_zeros);

%confidence interval
ci=Area+[-1 1].*(realsqrt(2)*erfcinv(alpha)*Serror);
%ci=Area+[-1 1].*(1.96*Serror);
...
if verbose
    ...
    plot(xroc_perf,yroc_perf,'-','LineWidth',3,'Color',[0.2 0.2 0.85]);
    H1=plot(xroc_perf,yroc_perf,'bo','LineWidth',1);
    set(H1,...
        'markersize',5,...
        'markeredgecolor',[0.2 0.2 0.85],...
        'markerfacecolor',[0.4 0.4 0.95])
    
      hold off
    xlabel('False positive rate (1-Specificity)')
    ylabel('True positive rate (Sensitivity)')
    title(sprintf('ROC curve (AUC = %0.4f, SE %0.4f)',Area,Serror),'FontName','Times New Roman','FontSize',14)
    axis square
end
%% Statistic evaluation
if p<=alpha
    % ButtonName = questdlg('Do you want to input the true prevalence?', 'Prevalence Question', 'Yes', 'No', 'Yes');
    ButtonName = 'NO'; % Hard coded this to stop asking [M]
   ...
   % Print the Table with cut-off values
        table=[labels'; a(:,1)'; a(:,2)';c';b(:,1)'; b(:,2)'; YounesI']';
        if verbose2
            disp('ROC CURVE DATA')
            disp(tr)
            fprintf('Cut-off \t\tSensitivity\tSpecificity\tEfficiency\tPLR\t\t\tNLR\t\t\tYounes Index\n')
            fprintf('%0.2e\t\t%0.4f\t\t%0.4f\t\t%0.4f\t\t%0.4f\t\t%0.4f\t\t%0.4f\n',table')
            disp(tr)
            disp(' ')
        end
    end
    
    % Calculates cut-off points
    CSe=table(:,1)==max(table(table(:,2)==max(table(:,2)))); %Max sensitivity cut-off < - changed min to max? 
    ...
    Yi=table(YounesI==max(YounesI),1); %[M] -------------<---
    xg=linspace(0,max(table(:,1)),500); %Dun now, not used
    %% 
    %{
...
%         axis([xg(1) xg(end) 0 1.1])
    ...
    %% Print Cut-Off Points
        fprintf('1) Max Sensitivity Cut-off point= %0.2e\n',table(CSe,1)) %mariochange
        fprintf('2) Max Specificity Cut-off point= %0.2e\n',table(CSp,1))
        fprintf('3) Cost Effective Cut-off point= %0.2e\n',CE), 
        fprintf('4) Max Efficiency Cut-off point= %0.2e\n',table(CEff,1))
        fprintf('5) [M] Max Younes Index Cut-off point= %0.2e\n',Yi), 
        m=[table(CSe,1) table(CSp,1) CE table(CEff,1)];
        m1=table;
        ml.Max_Sensitivity=table(CSe,1);
        ml.Max_Specificty=table(CSp,1);
        ml.Cost_Effective=CE;
        ml.Max_Efficiency=table(CEff,1);
        ml.MaxYounes=Yi;
else
    table=NaN;
end
...
if nargout
    ...
    try
    ROCout.cutoffPoitns=struct2table(ml); % cut off points
    catch
        ml=NaN(1,5);
    end
        
        
    ROCout.xroc=xroc; %graphic x points
    ROCout.yroc=yroc; %graphic y points
    
    ROCout.n_pos=n_ones; %true positives
    ROCout.n_neg=n_zeros; %true negatives
    
    %ROCout.xrF=xfit; %graphic xFit points
    %ROCout.yrF=yfit; %graphic yFit points
    ROCout.table=table; % [M] Force Table formation <-
    %ROCout.t2=table2struct(table);
    ROCout.YounesI=YounesI;
end

disp('Using ROC by Cardillo G. (2008) V1.33')
disp('D: Aug-10-2017')