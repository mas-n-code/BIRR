...
[darr,warrxy]=distcyl2D(length(Xn),Xn,Yn,xc,yc,theta);  %calls function "distcyl.m" to calculate distance 
       ...
       for i=1:Sloc 
           for n=1:Ttarg
                ...
                if i==j
                  ...
                  id(i,a,n)=sigma(n).*pat(warrxy(n,i)).*exp(-1i*2*pi*f(a)*(2*darr(n,i))./Mu)./(4*(darr(n,i)).^2);
                  
                else
                    
                 id(i,a,n)=sigma(n).*pat(warrxy(n,i)).*pat(warrxy(n,j)).*exp(-1i*2*pi*f(a)*(darr(n,i)+darr(n,j))./Mu)./(4*(darr(n,i)+darr(n,j)).^2);
                 
                end
                if abs(j-i)==54
                    vec=[vec darr(n,i)+darr(n,j)];
                    veci=[veci i];
                    vecj=[vecj j];
                end
