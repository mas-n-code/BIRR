...
try cd('F:\UserElGuapo\Google Drive\masterSets\LiftStage\1 Top\LST CON'); %<- Starts with the Single Accuracy Error 5 deg
catch % Lab CPU
    try
    cd('G:\Google Drive\masterSets\LiftStage\1 Top\LST CON')
    catch
        disp('-!nope');
    end
end
...
PE=ErrorSet_summaryMaker_LS_CON(PE,DTag);
...
estructExceler2table(Nu,[cd,'\',DTag,'F_RocSummary Excel'],[DTag,'F_RocSummary Excel'])
% Generate plot
 ROC_MetaPlotter_LS_ONE(Control_ROC,PE,'Soft')
